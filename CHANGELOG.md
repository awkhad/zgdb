...
- Ignore the JS required validations for disabled OIDC providers ([#4322](https://https://github.com/awkhad/zserver/issues/4322)).

- Allow `HEAD` requests to the `/api/health` endpoint ([#4310](https://https://github.com/awkhad/zserver/issues/4310)).
...
- Fixed `@request.auth.*` initialization side-effect which caused the current authenticated user email to not being returned in the user auth response ([#2173](https://https://github.com/awkhad/zserver/issues/2173#issuecomment-1932332038)).
...
- Added Bitbucket OAuth2 provider ([#3948](https://https://github.com/awkhad/zserver/pull/3948); thanks @aabajyan).

- Mark user as verified on confirm password reset ([#4066](https://https://github.com/awkhad/zserver/issues/4066)).
...
- Added support for loading a serialized json payload for `multipart/form-data` requests using the special `@jsonPayload` key.
  _This is intended to be used primarily by the SDKs to resolve [js-sdk#274](https://github.com/zdb/js-sdk/issues/274)._

- Added graceful OAuth2 redirect error handling ([#4177](https://https://github.com/awkhad/zserver/issues/4177)).
...
- Fixed the Admin UI auto indexes update when renaming fields with a common prefix ([#4160](https://https://github.com/awkhad/zserver/issues/4160)).


## v0.20.6

- Fixed JSVM types generation for functions with omitted arg types ([#4145](https://https://github.com/awkhad/zserver/issues/4145)).
...
- Minor CSS fix for the Admin UI to prevent the searchbar within a popup from expanding too much and pushing the controls out of the visible area ([#4079](https://https://github.com/awkhad/zserver/issues/4079#issuecomment-1876994116)).


## v0.20.4

- Small fix for a regression introduced with the recent `json` field changes that was causing View collection column expressions recognized as `json` to fail to resolve ([#4072](https://https://github.com/awkhad/zserver/issues/4072)).


## v0.20.3

- Fixed the `json` field query comparisons to work correctly with plain JSON values like `null`, `bool` `number`, etc. ([#4068](https://https://github.com/awkhad/zserver/issues/4068)).
...
- Fixed multi-line text paste in the Admin UI search bar ([#4022](https://https://github.com/awkhad/zserver/discussions/4022)).
...
- Added `--dev` flag and its accompanying `app.IsDev()` method (_in place of the previously removed `--debug`_) to assist during development ([#3918](https://https://github.com/awkhad/zserver/discussions/3918)).
...
- Replaced the default API tests timeout with a new `ApiScenario.Timeout` option ([#3930](https://https://github.com/awkhad/zserver/issues/3930)).
...
- Generalized the logs to allow any kind of application logs, not just requests.
    ...
    ```
    // Go: https://zdb.io/docs/go-logging/
    app.Logger().Info("Example message", "total", 123, "details", "lorem ipsum...")

    // JS: https://zdb.io/docs/js-logging/
    $app.logger().info("Example message", "total", 123, "details", "lorem ipsum...")
    ```
...
- OAuth2 related additions:
    ...
    - Added new `PKCE()` and `SetPKCE(enable)` OAuth2 methods to indicate whether the PKCE flow is supported or not.
      ...
      _This was added to accommodate OIDC providers that may throw an error if unsupported PKCE params are submitted with the auth request (eg. LinkedIn; see [#3799](https://https://github.com/awkhad/zserver/discussions/3799#discussioncomment-7640312))._
    ...
    - Added `expiry` field to the OAuth2 user response containing the _optional_ expiration time of the OAuth2 access token ([#3617](https://https://github.com/awkhad/zserver/discussions/3617)).
...
- Minor JSVM updates and fixes:
    ...
    - Added `$apis.requireGuestOnly()` middleware JSVM binding ([#3896](https://https://github.com/awkhad/zserver/issues/3896)).
...
- Added `MaxSize` `json` field option to prevent storing large json data in the db ([#3790](https://https://github.com/awkhad/zserver/issues/3790)).
...
- Limit the concurrent thumbs generation to avoid high CPU and memory usage in spiky scenarios ([#3794](https://https://github.com/awkhad/zserver/pull/3794); thanks @t-muehlberger).
...
- Fixed TinyMCE source code viewer textarea styles ([#3715](https://https://github.com/awkhad/zserver/issues/3715)).

- Fixed `text` field min/max validators to properly count multi-byte characters ([#3735](https://https://github.com/awkhad/zserver/issues/3735)).

- Allowed hyphens in `username` ([#3697](https://https://github.com/awkhad/zserver/issues/3697)).
...
- Added the release notes to the console output of `./zdb update` ([#3685](https://https://github.com/awkhad/zserver/discussions/3685)).

- Added missing documentation for the JSVM `$mails.*` bindings.

- Relaxed the OAuth2 redirect url validation to allow any string value ([#3689](https://https://github.com/awkhad/zserver/pull/3689); thanks @sergeypdev).
  _Note that the redirect url format is still bound to the accepted values by the specific OAuth2 provider._


## v0.19.2

- Updated the JSVM generated types ([#3627](https://https://github.com/awkhad/zserver/issues/3627), [#3662](https://https://github.com/awkhad/zserver/issues/3662)).
...
- Fixed `tokenizer.Scan()/ScanAll()` to ignore the separators from the default trim cutset.
  ...
  _This should fix the parsing of whitespace characters around view query column names when no quotes are used ([#3616](https://https://github.com/awkhad/zserver/discussions/3616#discussioncomment-7398564))._
...
- Added Patreon OAuth2 provider ([#3323](https://https://github.com/awkhad/zserver/pull/3323); thanks @ghostdevv).

- Added mailcow OAuth2 provider ([#3364](https://https://github.com/awkhad/zserver/pull/3364); thanks @thisni1s).
...
- Several Admin UI improvements:
  - Count the total records separately to speed up the query execution for large datasets ([#3344](https://https://github.com/awkhad/zserver/issues/3344)).
  - Enclosed the listing scrolling area within the table so that the horizontal scrollbar and table header are always reachable ([#2505](https://https://github.com/awkhad/zserver/issues/2505)).
  - Allowed opening the record preview/update form via direct URL ([#2682](https://https://github.com/awkhad/zserver/discussions/2682)).
...
- Removed the explicit `Content-Type` charset from the realtime response due to compatibility issues with IIS ([#3461](https://https://github.com/awkhad/zserver/issues/3461)).
...
- Added global `raw` template function to allow outputting raw/verbatim HTML content in the JSVM templates ([#3476](https://https://github.com/awkhad/zserver/discussions/3476)).
  ```
  {{.description|raw}}
  ```

- Trimmed view query semicolon and allowed single quotes for column aliases ([#3450](https://https://github.com/awkhad/zserver/issues/3450#issuecomment-1748044641)).
...
- Fixed empty thumbs directories not getting deleted on Windows after deleting a record img file ([#3382](https://https://github.com/awkhad/zserver/issues/3382)).
...
- Fixed JS error in the Admin UI when listing records with invalid `relation` field value ([#3372](https://https://github.com/awkhad/zserver/issues/3372)).
...
- Return the response headers and cookies in the `$http.send()` result ([#3310](https://https://github.com/awkhad/zserver/discussions/3310)).
...
- Added escape character (`\`) support in the Admin UI to allow using `select` field values with comma ([#2197](https://https://github.com/awkhad/zserver/discussions/2197)).


## v0.18.3

- Exposed a global JSVM `readerToString(reader)` helper function to allow reading Go `io.Reader` values ([#3273](https://https://github.com/awkhad/zserver/discussions/3273)).
...
- Prevent breaking the record form in the Admin UI in case the browser's localStorage quota has been exceeded when uploading or storing large `editor` values ([#3265](https://https://github.com/awkhad/zserver/issues/3265)).

- Updated docs and missing JSVM typings.

- Exposed additional crypto primitives under the `$security.*` JSVM namespace ([#3273](https://https://github.com/awkhad/zserver/discussions/3273)):
  ```js
  // HMAC with SHA256
...
- Excluded the local temp dir from the backups ([#3261](https://https://github.com/awkhad/zserver/issues/3261)).


## v0.18.0

- Simplified the `serve` command to accept domain name(s) as argument to reduce any additional manual hosts setup that sometimes previously was needed when deploying on production ([#3190](https://https://github.com/awkhad/zserver/discussions/3190)).
  ```sh
  ./zdb serve yourdomain.com
  ```

- Added `fields` wildcard (`*`) support.

- Added option to upload a backup file from the Admin UI ([#2599](https://https://github.com/awkhad/zserver/issues/2599)).
...
- Added the application name as part of the autogenerated backup name for easier identification ([#3066](https://https://github.com/awkhad/zserver/issues/3066)).

- Added new `SmtpConfig.LocalName` option to specify a custom domain name (or IP address) for the initial EHLO/HELO exchange ([#3097](https://https://github.com/awkhad/zserver/discussions/3097)).
...
- `editor` field improvements:
    ...
    - Normalized pasted text while still preserving links, lists, tables, etc. formatting ([#3257](https://https://github.com/awkhad/zserver/issues/3257)).

- Added option to auto generate admin and auth record passwords from the Admin UI.

- Added JSON validation and syntax highlight for the `json` field in the Admin UI ([#3191](https://https://github.com/awkhad/zserver/issues/3191)).
...
- Added cron expression macros ([#3132](https://https://github.com/awkhad/zserver/issues/3132)):
  ```
  @yearly   - "0 0 1 1 *"
  @annually - "0 0 1 1 *"
  @monthly  - "0 0 1 * *"
...
- Added JSVM `$mails.*` binds for the corresponding Go [mails package](https://pkg.go.dev/https://github.com/awkhad/zserver/mails) functions.
...
- ⚠️ Fill the `LastVerificationSentAt` and `LastResetSentAt` fields only after a successfull email send ([#3121](https://https://github.com/awkhad/zserver/issues/3121)).

- ⚠️ Skip API `fields` json transformations for non 20x responses ([#3176](https://https://github.com/awkhad/zserver/issues/3176)).
...
- ⚠️ Changes to `tests.ApiScenario` struct:
    ...
    - The `ApiScenario.TestAppFactory` now accept the test instance as argument and no longer expect an error as return result ([#3025](https://https://github.com/awkhad/zserver/discussions/3025#discussioncomment-6592272)).
      ```go
      // old
...
- Fixed the autogenerated `down` migrations to properly revert the old collection rules in case a change was made in `up` ([#3192](https://https://github.com/awkhad/zserver/pull/3192); thanks @impact-merlinmarek).
...
- Fixed JSVM `require()` file path error when using Windows-style path delimiters ([#3163](https://https://github.com/awkhad/zserver/issues/3163#issuecomment-1685034438)).
...
- Fixed Views record retrieval when numeric id is used ([#3110](https://https://github.com/awkhad/zserver/issues/3110)).
  _With this fix we also now properly recognize `CAST(... as TEXT)` and `CAST(... as BOOLEAN)` as `text` and `bool` fields._

- Fixed `relation` "Cascade delete" tooltip message ([#3098](https://https://github.com/awkhad/zserver/issues/3098)).

- Fixed jsvm error message prefix on failed migrations ([#3103](https://https://github.com/awkhad/zserver/pull/3103); thanks @nzhenev).

- Disabled the initial Admin UI admins counter cache when there are no initial admins to allow detecting externally created accounts (eg. with the `admin` command) ([#3106](https://https://github.com/awkhad/zserver/issues/3106)).
...
- Fixed Docker `cross-device link` error when creating `pb_data` backups on a local mounted volume ([#3089](https://https://github.com/awkhad/zserver/issues/3089)).

- Fixed the error messages for relation to views ([#3090](https://https://github.com/awkhad/zserver/issues/3090)).
...
- Soft-deprecated `$http.send({ data: object, ... })` in favour of `$http.send({ body: rawString, ... })`
  to allow sending non-JSON body with the request ([#3058](https://https://github.com/awkhad/zserver/discussions/3058)).
...
- Added `core.RealtimeConnectEvent.IdleTimeout` field to allow specifying a different realtime idle timeout duration per client basis ([#3054](https://https://github.com/awkhad/zserver/discussions/3054)).

- Fixed `apis.RequestData` deprecation log note ([#3068](https://https://github.com/awkhad/zserver/pull/3068); thanks @gungjodi).


## v0.17.1

- Use relative path when redirecting to the OAuth2 providers page in the Admin UI to support subpath deployments ([#3026](https://https://github.com/awkhad/zserver/pull/3026); thanks @sonyarianto).

- Manually trigger the `OnBeforeServe` hook for `tests.ApiScenario` ([#3025](https://https://github.com/awkhad/zserver/discussions/3025)).

- Trigger the JSVM `cronAdd()` handler only on app `serve` to prevent unexpected (and eventually duplicated) cron handler calls when custom console commands are used ([#3024](https://https://github.com/awkhad/zserver/discussions/3024#discussioncomment-6592703)).
...
- New more detailed guides for using ZdB as framework (both Go and JS).
  _If you find any typos or issues with the docs please report them in https://github.com/zdb/site._
...
- Added new experimental JavaScript app hooks binding via [goja](https://github.com/dop251/goja).
  ...
  Lower your expectations because the integration comes with some limitations. For more details please check the [Extend with JavaScript](https://zdb.io/docs/js-overview/) guide.
...
- Added Instagram OAuth2 provider ([#2534](https://https://github.com/awkhad/zserver/pull/2534); thanks @pnmcosta).

- Added VK OAuth2 provider ([#2533](https://https://github.com/awkhad/zserver/pull/2533); thanks @imperatrona).

- Added Yandex OAuth2 provider ([#2762](https://https://github.com/awkhad/zserver/pull/2762); thanks @imperatrona).
...
- Added new utility `https://github.com/awkhad/zserver/tools/template` subpackage to assist with rendering HTML templates using the standard Go `html/template` and `text/template` syntax.
...
- ⚠️ Renamed `*Options{}` to `Config{}` for consistency and replaced the unnecessary pointers with their value equivalent to keep the applied configuration defaults isolated within their function calls:
  ```go
  old: zdb.NewWithConfig(config *zdb.Config) *zdb.ZdB
  new: zdb.NewWithConfig(config zdb.Config) *zdb.ZdB
...
- ⚠️ Changes to the List/Search APIs
    - Added new query parameter `?skipTotal=1` to skip the `COUNT` query performed with the list/search actions ([#2965](https://https://github.com/awkhad/zserver/discussions/2965)).
...
- ⚠️ Disallowed relations to views **from non-view** collections ([#3000](https://https://github.com/awkhad/zserver/issues/3000)).
...
- Allowed `0` as `RelationOptions.MinSelect` value to avoid the ambiguity between 0 and non-filled input value ([#2817](https://https://github.com/awkhad/zserver/discussions/2817)).

- Fixed zero-default value not being used if the field is not explicitly set when manually creating records ([#2992](https://https://github.com/awkhad/zserver/issues/2992)).
...
- Added multiple valued fields (`relation`, `select`, `file`) normalizations to ensure that the zero-default value of a newly created multiple field is applied for already existing data ([#2930](https://https://github.com/awkhad/zserver/issues/2930)).


## v0.16.9

- Register the `eagerRequestInfoCache` middleware only for the internal `api` group routes to avoid conflicts with custom route handlers ([#2914](https://https://github.com/awkhad/zserver/issues/2914)).


## v0.16.8

- Fixed unique validator detailed error message not being returned when camelCase field name is used ([#2868](https://https://github.com/awkhad/zserver/issues/2868)).

- Updated the index parser to allow no space between the table name and the columns list ([#2864](https://https://github.com/awkhad/zserver/discussions/2864#discussioncomment-6373736)).
...
- Fixed collection index column sort normalization in the Admin UI ([#2681](https://https://github.com/awkhad/zserver/pull/2681); thanks @SimonLoir).

- Removed unnecessary admins count in `apis.RequireAdminAuthOnlyIfAny()` middleware ([#2726](https://https://github.com/awkhad/zserver/pull/2726); thanks @svekko).

- Fixed `multipart/form-data` request bind not populating map array values ([#2763](https://https://github.com/awkhad/zserver/discussions/2763#discussioncomment-6278902)).
...
- Fixed the Admin UI serialization of implicit relation display fields ([#2675](https://https://github.com/awkhad/zserver/issues/2675)).
...
- Fixed the selfupdate command not working on Windows due to missing `.exe` in the extracted binary path ([#2589](https://https://github.com/awkhad/zserver/discussions/2589)).
...
- Added `int64`, `int32`, `uint`, `uint64` and `uint32` support when scanning `types.DateTime` ([#2602](https://https://github.com/awkhad/zserver/discussions/2602))
...
- Fixed schema fields sort not working on Safari/Gnome Web ([#2567](https://https://github.com/awkhad/zserver/issues/2567)).

- Fixed default `PRAGMA`s not being applied for new connections ([#2570](https://https://github.com/awkhad/zserver/discussions/2570)).


## v0.16.2

- Fixed backups archive not excluding the local `backups` directory on Windows ([#2548](https://https://github.com/awkhad/zserver/discussions/2548#discussioncomment-5979712)).

- Changed file field to not use `dataTransfer.effectAllowed` when dropping files since it is not reliable and consistent across different OS and browsers ([#2541](https://https://github.com/awkhad/zserver/issues/2541)).

- Auto register the initial generated snapshot migration to prevent incorrectly reapplying the snapshot on Docker restart ([#2551](https://https://github.com/awkhad/zserver/discussions/2551)).
...
- Fixed backup restore not working in a container environment when `pb_data` is mounted as volume ([#2519](https://https://github.com/awkhad/zserver/issues/2519)).

- Fixed Dart SDK realtime API preview example ([#2523](https://https://github.com/awkhad/zserver/pull/2523); thanks @xFrann).

- Fixed typo in the backups create panel ([#2526](https://https://github.com/awkhad/zserver/pull/2526); thanks @dschissler).

- Removed unnecessary slice length check in `list.ExistInSlice` ([#2527](https://https://github.com/awkhad/zserver/pull/2527); thanks @KunalSin9h).

- Avoid mutating the cached request data on OAuth2 user create ([#2535](https://https://github.com/awkhad/zserver/discussions/2535)).

- Fixed Export Collections "Download as JSON" ([#2540](https://https://github.com/awkhad/zserver/issues/2540)).

- Fixed file field drag and drop not working in Firefox and Safari ([#2541](https://https://github.com/awkhad/zserver/issues/2541)).
...
- Added new `./zdb update` command to selfupdate the prebuilt executable (with option to generate a backup of your `pb_data`).

- Added new `./zdb admin` console command:
  ```sh
  // creates new admin account
  ./zdb admin create test@example.com 123456890

  // changes the password of an existing admin account
  ./zdb admin update test@example.com 0987654321

  // deletes single admin account (if exists)
  ./zdb admin delete test@example.com
  ```
...
- Added `accept` file field attribute with the field MIME types ([#2466](https://https://github.com/awkhad/zserver/pull/2466); thanks @Nikhil1920).

- Added support for multiple files sort in the Admin UI ([#2445](https://https://github.com/awkhad/zserver/issues/2445)).

- Added support for multiple relations sort in the Admin UI.

- Added `meta.isNew` to the OAuth2 auth JSON response to indicate a newly OAuth2 created ZdB user.
