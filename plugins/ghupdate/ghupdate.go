...
// ZdB executable with the latest GitHub release.
//
// Example usage:
//
...
import (
	...
	"https://github.com/awkhad/zserver/tools/archive"
...
type Config struct {
	// Owner specifies the account owner of the repository (default to "zdb").
	Owner string

	// Repo specifies the name of the repository (default to "zdb").
	Repo string

	// ArchiveExecutable specifies the name of the executable file in the release archive
	// (default to "zdb"; an additional ".exe" check is also performed as a fallback).
...
func Register(app core.App, rootCmd *cobra.Command, config Config) error {
	...
	if p.config.ArchiveExecutable == "" {
		p.config.ArchiveExecutable = "zdb"
	}
...
func (p *plugin) update(withBackup bool) error {
	...
	if latest.Body != "" {
		...
		releaseNotes := strings.TrimSpace(strings.Replace(latest.Body, "> _To update the prebuilt executable you can run `./zdb update`._", "", 1))
